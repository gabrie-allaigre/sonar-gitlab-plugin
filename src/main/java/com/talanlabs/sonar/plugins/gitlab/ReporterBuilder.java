...
public class ReporterBuilder {
    ...
    private void updateReviewCommentsPerInline(String revision, String username, File file, Integer lineNumber, List<ReportIssue> reportIssues) {
        ...
        String body = new InlineCommentBuilder(gitLabPluginConfiguration, revision, username, lineNumber, reportIssues, markDownUtils, analysisMode, dashboardUrl).buildForMarkdown();
    ...
    private void updateGlobalComments(QualityGate qualityGate, Reporter report) {
        ...
        String body = new GlobalCommentBuilder(gitLabPluginConfiguration, username, qualityGate, report, markDownUtils, analysisMode, dashboardUrl).buildForMarkdown();
        if (body != null && !body.trim().isEmpty()) {
